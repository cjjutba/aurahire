  ...
  async generateStructured<T>(
    ...
    try {
      const completion = await this.client.chat.completions.parse({
        ...
        response_format: zodResponseFormat(opts.schema as z.ZodTypeAny, opts.schemaName),
      });
...
export class RedactPiiService {
  ...
  private async scrubText(text: string, requestId?: string): Promise<string> {
    const result = await this.openai.generateText({
      ...
      requestId: requestId ? `${requestId}:redact-v${REDACT_TEXT_VERSION}` : undefined,
    });
...
async function main() {
  ...
  if (parseResult.data.skills.length === 0) {
    throw new Error("Parse result has zero skills — unexpected for the test resume");
  }
