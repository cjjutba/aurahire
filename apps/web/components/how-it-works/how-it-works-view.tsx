...
import { HelpBlockRenderer } from "../help/help-block";
import type { HelpBlock } from "../help/help-types";
import { adminHowItWorks } from "./content/admin-content";
import { candidateHowItWorks } from "./content/candidate-content";
import { recruiterHowItWorks } from "./content/recruiter-content";
import type {
  HowItWorksContent,
...
export type HowItWorksVariant = "recruiter" | "candidate" | "admin";

const HOW_IT_WORKS_CONTENT: Record<HowItWorksVariant, HowItWorksContent> = {
  recruiter: recruiterHowItWorks,
  candidate: candidateHowItWorks,
  admin: adminHowItWorks,
};
...
export function HowItWorksView({ variant }: HowItWorksViewProps) {
  const content = HOW_IT_WORKS_CONTENT[variant];
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return { groups: content.groups };

    const filterSection = (s: HowItWorksSection): boolean => {
...
function JourneyStrip({
  ...
  return (
    <section
      aria-label={title}
      className="mb-10 rounded-[var(--radius-lg)] border border-[var(--color-hairline)] bg-[var(--color-surface-soft)]"
    >
      <header className="flex items-center gap-2 border-b border-[var(--color-hairline-soft)] px-5 py-3">
        <span className="text-[11px] font-semibold uppercase tracking-wider text-[var(--color-muted)]">
          {title}
        </span>
      </header>
      <ol className="relative flex snap-x snap-mandatory gap-3 overflow-x-auto px-5 py-5 [scrollbar-color:var(--color-hairline)_transparent] [scrollbar-width:thin]">
        {steps.map((step, idx) => {
          ...
          return (
            <li
              key={idx}
              className="relative flex min-w-[180px] flex-1 snap-start items-start gap-3"
            >
              {clickable ? (
                <button
                  type="button"
                  onClick={() => onPick(targetId)}
                  className="group flex w-full items-start gap-3 rounded-[var(--radius-md)] border border-[var(--color-hairline-soft)] bg-[var(--color-canvas)] p-3 text-left transition hover:border-[var(--color-primary-soft)] hover:bg-[var(--color-primary-soft)]/30"
                >
                  <JourneyStepBody
                    Icon={Icon}
                    idx={idx}
                ...
                <div className="group flex w-full items-start gap-3 rounded-[var(--radius-md)] border border-[var(--color-hairline-soft)] bg-[var(--color-canvas)] p-3">
                  <JourneyStepBody
                    Icon={Icon}
                    idx={idx}
                    label={step.label}
              ...
              {idx < steps.length - 1 && (
                <ArrowRight
                  className="absolute -right-2 top-5 hidden h-3 w-3 shrink-0 text-[var(--color-muted-soft)] sm:block"
                  aria-hidden
                />
              )}
            </li>
...
function JourneyStepBody({
  ...
  Icon: HowItWorksJourneyStep["icon"];
  idx: number;
  label: string;
  description: string;
}) {
  return (
    <>
      <span className="relative mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[var(--color-primary-soft)]">
        <Icon className="h-4 w-4 text-[var(--color-primary)]" />
        <span className="absolute -right-1 -top-1 inline-flex h-4 w-4 items-center justify-center rounded-full border border-[var(--color-hairline)] bg-[var(--color-canvas)] font-mono text-[9px] font-semibold text-[var(--color-muted)]">
          {idx + 1}
        </span>
      </span>
      <div className="min-w-0 flex-1 space-y-0.5">
        ...
        <div className="text-xs leading-5 text-[var(--color-body)]">
          {description}
        </div>
      </div>
    </>
...
function SectionView({ section }: { section: HowItWorksSection }) {
...
function TocList({
  ...
  groups: HowItWorksContent["groups"];
...
function ContactCard({ contact }: { contact: HowItWorksContent["contact"] }) {
  return (
    <section
      id="contact"
...
function blockMatches(block: HelpBlock, q: string): boolean {
  switch (block.kind) {
    case "paragraph":
