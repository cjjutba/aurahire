...
const SORT_OPTIONS = [
  ...
  { value: "recent", label: "Recently Scheduled" },
];
...
export function InterviewsToolbarClient({
  q,
  status,
  format,
  sort,
  ...
  const updateParam = useCallback(
    (key: string, value: string) => {
      ...
      startTransition(() => {
        router.push(
          `/recruiter/interviews${params.toString() ? `?${params.toString()}` : ""}`,
        );
      });
    },
    [router, searchParams],
  ...
  const currentFormat =
    FORMAT_OPTIONS.find((o) => o.value === format) ?? FORMAT_OPTIONS[0]!;
  const currentSort =
  ...
  return (
    ...
    >
      ...
      <FilterDropdown
        label="Format"
        current={currentFormat.label}
        options={FORMAT_OPTIONS}
        onSelect={(v) => updateParam("format", v)}
      />
