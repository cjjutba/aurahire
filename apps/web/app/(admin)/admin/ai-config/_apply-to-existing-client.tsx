  ...
  async function pollOnce(queueJobId: string) {
    try {
      ...
      if (body.data.state === "completed") {
        ...
        toastSuccess(
          "Rescore complete",
          `${r?.processedCount ?? 0} applications updated.`,
        );
      } else if (body.data.state === "failed") {
        stopPolling();
        toastApiError(
          null,
          "Couldn't complete rescore",
          body.data.failedReason ?? "(no reason)",
        );
        ...
        toastApiError(
          null,
          "Couldn't complete rescore",
          "Check the audit log for completion details.",
        );
      }
    } catch (err) {
      stopPolling();
  ...
  async function enqueue() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toastApiError(
          null,
          "Couldn't queue rescore",
          body.message ?? "Queue may be unavailable. Verify Redis is running.",
        );
        return;
      }
