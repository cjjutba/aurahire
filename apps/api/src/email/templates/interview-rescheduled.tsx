...
export interface InterviewRescheduledEmailProps {
  candidateName: string;
  jobTitle: string;
  companyName: string;
  previousScheduledAt: string; // ISO
...
export function InterviewRescheduledEmail({
  ...
  applicationUrl,
  company,
}: InterviewRescheduledEmailProps): React.ReactElement {
  const previousWhen = new Date(previousScheduledAt).toLocaleString();
  const newWhen = new Date(newScheduledAt).toLocaleString();
  ...
  return (
    <Html>
      <Head />
      <Preview>{`Interview rescheduled: ${jobTitle} at ${companyName}`}</Preview>
      <Body
        style={{
